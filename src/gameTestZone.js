...
// Full list of items to find in game / on level
const gameItemsList = [
    {itemName: 'Firework', attackAdd: 10, itemQuantity: 1},
    {itemName: 'Mobile Phone', signal: false, itemQuantity: 1},
    {itemName: 'DNA Sample', value: 1000000, itemQuantity: 1},
...
// List of random dinosours who might attack you
const dinosaurs = [
    'T-Rex', 
    'Bronte-Sisters-ours',
...
// Dinosaur attack function. Lose a random amount of health
// Dinos attack x amount of times and do y damage each time
// When attack there is a random chance for items to be dropped
function attack() {
    if (paleontologist.health <= 200 && paleontologist.health >= 50) {
        ...
        // random item drop sections
        // index number to search item array 
        randomItemDropped = 0
...
// Find random weapon and push to weapon array
...
// climb electric fence
// canTakeVoltage needs to be 50000+ or will go down to 1 health
// might need a way to equip the gloves
// gloves are lost after using
...
// PRACTICE FUNCTION 
// To go inside hunt for items. 
// An attempt at checking invitintory for random items. 
function include() {
...
function huntForItems() {
    ...
    if (Math.floor(Math.random() * 10) + 1 >= 2) {
        ...
        for (let i = 0; i < paleontologist.invintory.length; i++) {
            // if 
            if (paleontologist.invintory[i].itemName.includes(paleontologist.invintory[randomItemFound].itemName)) {
                console.log('hello')
                paleontologist.invintory[i].itemQuantity += foundItemQuantity
            } else {
                paleontologist.invintory.push(gameItemsList[randomItemFound])
                console.log('end ', paleontologist.invintory)
                return
            }
            // IF name of item matches name in invintory add to quantity 
            // IF it doesnt match add item and found quantity to invintory
                // doesnt not match then push to invintory

            /// USED TO BE THE INCLUDE FUNCTION - Does work sort of
